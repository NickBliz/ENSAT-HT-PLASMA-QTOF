...
X=X[which(EQP_METADATA_POS$GROUP=="PHT"),]
WHYF=data.frame(EQP_METADATA_POS[which(EQP_METADATA_POS$GROUP=="PHT"),])
pca.EQP = pca(X, ncomp = 10, center = TRUE, scale = FALSE)
plotIndiv(pca.EQP, group = WHYF$CENTER.ID, title = "POS QTOF", ind.names = F,legend = T,cex=1, pch = "sphere", style = "3d")
...
X=EQP_POS
row.names(X)=EQP_METADATA_POS$`sample name 2`
X=X[-which(EQP_METADATA_POS$GROUP=="X" | EQP_METADATA_POS$GROUP=="Qc Nick" | EQP_METADATA_POS$GROUP=="QC plasmamonster" | EQP_METADATA_POS$GROUP=="HV"),]
