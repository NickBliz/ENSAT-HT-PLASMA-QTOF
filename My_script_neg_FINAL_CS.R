X=EQP_NEG[-which(EQP_METADATA_NEG$GROUP=="PA" | EQP_METADATA_NEG$GROUP=="PPGL" | EQP_METADATA_NEG$GROUP=="X" | EQP_METADATA_NEG$GROUP=="Qc Nick" | EQP_METADATA_NEG$GROUP=="QC plasmamonster" | EQP_METADATA_NEG$GROUP=="HV"),]
pca.EQP = pca(X, ncomp = 10, center = TRUE, scale = FALSE)
plotIndiv(pca.EQP, group = EQP_METADATA_NEG$GROUP[-which(EQP_METADATA_NEG$GROUP=="PA" | EQP_METADATA_NEG$GROUP=="PPGL" | EQP_METADATA_NEG$GROUP=="X" | EQP_METADATA_NEG$GROUP=="Qc Nick" | EQP_METADATA_NEG$GROUP=="QC plasmamonster" | EQP_METADATA_NEG$GROUP=="HV")], title = "NEG QTOF", ind.names = F,legend = T,cex=4, pch = "sphere", style = "3d")
plotIndiv(pca.EQP, group = EQP_METADATA_NEG$`CENTER ID`[-which(EQP_METADATA_NEG$GROUP=="PA" | EQP_METADATA_NEG$GROUP=="PPGL" | EQP_METADATA_NEG$GROUP=="X" | EQP_METADATA_NEG$GROUP=="Qc Nick" | EQP_METADATA_NEG$GROUP=="QC plasmamonster" | EQP_METADATA_NEG$GROUP=="HV")], title = "NEG QTOF", ind.names = F,legend = T,cex=4, pch = "sphere", style = "3d")

WHYF=data.frame(EQP_METADATA_NEG[-which(EQP_METADATA_NEG$GROUP=="PA" | EQP_METADATA_NEG$GROUP=="PPGL" | EQP_METADATA_NEG$GROUP=="X" | EQP_METADATA_NEG$GROUP=="Qc Nick" | EQP_METADATA_NEG$GROUP=="QC plasmamonster" | EQP_METADATA_NEG$GROUP=="HV"),])
...
WHYF=WHYF[which(WHYF$GROUP=="CS" | WHYF$GROUP=="PHT"),]
ex=X
Y=WHYF$GROUP
length(which(Y=="CS"))
length(which(Y=="PHT"))
...
set.seed(1)  #for keeping perf the same as with lapply
perf.plsda.select <- perf(plsda.select, validation = "Mfold", folds = f, auc = F, progressBar = T, nrepeat = 50, dist="mahalanobis.dist")
ncomp=perf.plsda.select$choice.ncomp[2,]
set.seed(1)
tune.splsda.select <- tune.splsda(X, Y, ncomp = ncomp, validation = "Mfold", folds = f, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 50, scale = F)
...
B.hat_CSVPHT1=test.predict$B.hat[,,ncomp]
rich=B.hat_CSVPHT1[,1]
...
CSVPHT1=list(plsda.select, splsda.select, perf.plsda.select, tune.splsda.select, rich)
CSVPHT2=list(allconf, all.BER, all.CS, all.PHT)
...
Y[which(Y=="PHT")]="0"
set.seed(1)  #for keeping perf the same as with lapply
cvfit1 = cv.glmnet(X, Y, family = "binomial", alpha=1, nfolds = f, type.measure = "class", standardize=FALSE)
rich=coef(cvfit1, s = "lambda.min")
rich=rich[-1,]
rich=rich[which(rich>0 | rich<0)]
rich=as.matrix(rich)
...
CSVPHT3=list(cvfit1, rich)
CSVPHT4=list(allconf, all.BER, all.CS, all.PHT)
...
ncomp=perf.plsda.select$choice.ncomp[2,]
set.seed(1)
tune.splsda.select <- tune.splsda(X, Y, ncomp = ncomp, validation = "Mfold", folds = f, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 50, scale = T)
ncomp=tune.splsda.select$choice.ncomp$ncomp
splsda.select <- splsda(X, Y, ncomp = ncomp, scale = T, keepX = tune.splsda.select$choice.keepX)
rich=vip(splsda.select)
rich[which(rowSums(rich)>0),]
test.predict <- predict(splsda.select, X, dist = "mahalanobis.dist")
B.hat_CSVPHT2=test.predict$B.hat[,,ncomp]
rich=B.hat_CSVPHT2[,1]
rich=rich[which(rich>0 | rich<0)]
rich=as.matrix(rich)
rich
rich_names=FEATURE_NAMES[,which(FEATURE_NAMES[1,] %in% row.names(rich))]
...
CSVPHT5=list(plsda.select, splsda.select, perf.plsda.select, tune.splsda.select, rich)
CSVPHT6=list(allconf, all.BER, all.CS, all.PHT)
...
Y[which(Y=="PHT")]="0"
set.seed(1)  #for keeping perf the same as with lapply
cvfit1 = cv.glmnet(X, Y, family = "binomial", alpha=1, nfolds = f, type.measure = "class", standardize=T)
rich=coef(cvfit1, s = "lambda.min")
rich=rich[-1,]
rich=rich[which(rich>0 | rich<0)]
rich=as.matrix(rich)
...
CSVPHT7=list(cvfit1, rich)
CSVPHT8=list(allconf, all.BER, all.CS, all.PHT)
