...
add_blank_matches <- function(samplesFilePath,pblanksFilePath) {
  ...
  blank_max3 <-rep(0.0, nrow(samples))
  blank_max4 <- rep(0.0, nrow(samples))
  ...
  difmin3 <- rep(0.0, nrow(samples))
  difmedian3 <- rep(0.0, nrow(samples))
  difmin4 <- rep(0.0, nrow(samples))
  difmedian4 <- rep(0.0, nrow(samples))
  bf <- rep(0.0, nrow(samples))
  bf2 <- rep(0.0, nrow(samples))
  bf3 <- rep(0.0, nrow(samples))
  bf4 <- rep(0.0, nrow(samples))
    ...
    for (j in 1:nrow(pblanks)) {
      ...
      if (blank_max[i]==0) {
        ...
        if (blank_max2[i]==0) {
          ...
          if (blank_max3[i]==0) {
            blank_max3[i] <- max(pblanks[j,] %>% select(starts_with("QT_")))
            difmin3[i] <- sample_min[i]-blank_max3[i]
            difmedian3[i] <- sample_median[i]-blank_max3[i]
            bf3[i] <- sample_min[i]-(3*blank_max3[i])
          } else {
            if (blank_max4[i]==0) {
              blank_max4[i] <- max(pblanks[j,] %>% select(starts_with("QT_")))
              difmin4[i] <- sample_min[i]-blank_max4[i]
              difmedian4[i] <- sample_median[i]-blank_max4[i]
              bf4[i] <- sample_min[i]-(3*blank_max4[i])
        }
  ...
  matched <<- cbind(samples, sample_min0, sample_median, sample_min, blank_pvalue, blank_max, difmin, difmedian, blank_pvalue2, blank_max2, difmin2, difmedian2, blank_matches, bf, bf2, bf3, bf4)
